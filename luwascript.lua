...
local AF_VEL_THRESHOLD   = 400   -- studs/s magnitude above this = fling detected
...
local function startAntiFling()
    ...
    -- Seed last safe position from current position
    ...
    afHeartbeatConn = RunService.Heartbeat:Connect(function()
        ...
        pcall(function()
            ...
            -- 1. Update last safe position when not being flung
            if vel < AF_VEL_THRESHOLD then
                lastSafePos = hrp.Position
            else
                -- 2. Fling detected: teleport back and put in GettingUp state
                if lastSafePos then
            ...
            local function buildServersWin()
                ...
                local function showPlayerPanel(jobId)
                    ...
                    task.spawn(function()
                        pcall(function()
                            ...
                            -- Fetch server players via thumbnail API using job ID approach
                            local url = "https://games.roblox.com/v1/games/" .. tostring(game.PlaceId)
                                     .. "/servers/" .. jobId .. "?limit=100"
                            local raw = game:HttpGet(url)
                            local data = HttpService:JSONDecode(raw)
                            local playerTokens = data.playerTokens or {}

                            -- Resolve tokens to user info
                            if #playerTokens == 0 then
                                ppTitle.Text = "No players found"
                                return
                            end
                            ...
                            local thumbData = HttpService:JSONDecode(thumbRaw)

                            ppTitle.Text = "Players (" .. #playerTokens .. ")"
                            ...
                            for i, tok in ipairs(playerTokens) do
                                ...
                                local imgUrl = ""
                                for _, td in ipairs(thumbData.data or {}) do
                                    if td.requestId == tok then imgUrl = td.imageUrl or ""; break end
                                end
                ...
                local function refreshLiveCounts()
                    ...
                    task.spawn(function()
                        pcall(function()
                            local HttpService = game:GetService("HttpService")
                            -- Fetch ALL loaded pages to get accurate counts for full servers too
                            ...
                            repeat
                                ...
                                for _, srv in ipairs(data.data or {}) do
                                    if srv.id then liveData[srv.id] = srv.playing or 0 end
                                end
                                cursor = data.nextPageCursor
                                fetched = fetched + 1
                            until (not cursor or cursor == "" or fetched >= 5)
                            updateVisibleCards()
                        end)
                    end)
                end
