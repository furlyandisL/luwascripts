...
local savedCmdWinPos            = nil  -- {xs,xo,ys,yo} for commands browser window
local savedSrvPos               = nil  -- {xs,xo,ys,yo} for servers window
...
local savedCmdWinScale          = nil  -- {w,h} for commands browser window size
local saveWinPositions          = true   -- save window positions to info.txt
...
local function loadSavedInfo()
    pcall(function()
        ...
        if t["save_win_pos"] == "false" then
            saveWinPositions = false
        end
        ...
        local cxs,cxo,cys,cyo = tonumber(t["cmd_xs"]),tonumber(t["cmd_xo"]),tonumber(t["cmd_ys"]),tonumber(t["cmd_yo"])
        if cxs and cxo and cys and cyo then
            savedCmdWinPos = { xs=cxs, xo=cxo, ys=cys, yo=cyo }
        end
        local sxs,sxo,sys,syo = tonumber(t["srv_xs"]),tonumber(t["srv_xo"]),tonumber(t["srv_ys"]),tonumber(t["srv_yo"])
        if sxs and sxo and sys and syo then
            savedSrvPos = { xs=sxs, xo=sxo, ys=sys, yo=syo }
        end
        ...
        local cw, ch = tonumber(t["cmd_sw"]), tonumber(t["cmd_sh"])
        if cw and ch and cw >= 200 and ch >= 180 then
            savedCmdWinScale = { w=cw, h=ch }
        end
        if t["save_win_sizes"] == "true" then
            saveWinSizes = true
        end
...
-- /windowsreset chat commands
-- /windowsreset       -> clears all saved positions AND sizes
-- /windowsreset pos   -> clears saved positions only
-- /windowsreset size  -> clears saved sizes only
LocalPlayer.Chatted:Connect(function(msg)
    local cmd = msg:lower():match("^%s*(.-)%s*$")
    if cmd == "/windowsreset" or cmd == "/windowsreset pos" or cmd == "/windowsreset size" then
        local doPos  = (cmd == "/windowsreset" or cmd == "/windowsreset pos")
        local doSize = (cmd == "/windowsreset" or cmd == "/windowsreset size")
        local patch  = {}
        ...
        if doPos then
            ...
            savedCmdWinPos  = nil
            savedSrvPos     = nil
            patch.pos_xs=DELETE;     patch.pos_xo=DELETE;     patch.pos_ys=DELETE;     patch.pos_yo=DELETE
            patch.picker_xs=DELETE;  patch.picker_xo=DELETE;  patch.picker_ys=DELETE;  patch.picker_yo=DELETE
            patch.profile_xs=DELETE; patch.profile_xo=DELETE; patch.profile_ys=DELETE; patch.profile_yo=DELETE
            patch.cmd_xs=DELETE;     patch.cmd_xo=DELETE;     patch.cmd_ys=DELETE;     patch.cmd_yo=DELETE
            patch.srv_xs=DELETE;     patch.srv_xo=DELETE;     patch.srv_ys=DELETE;     patch.srv_yo=DELETE
        end
        ...
        if doSize then
            ...
            savedCmdWinScale  = nil
            patch.scale_w=DELETE;    patch.scale_h=DELETE
            patch.picker_sw=DELETE;  patch.picker_sh=DELETE
            patch.profile_sw=DELETE; patch.profile_sh=DELETE
            patch.cmd_sw=DELETE;     patch.cmd_sh=DELETE
        end

        patchInfo(patch)

        local what = (doPos and doSize) and "positions & sizes"
                  or doPos              and "positions"
                  or                        "sizes"
        showNotify("notification", "Window " .. what .. " reset")
    end
end)
...
local function buildLauncherCircle()
    ...
    local HOLD_TIME  = 0.4   -- seconds before drag unlocks
    local TAP_MAX_PX = 12    -- if finger drifts beyond this before timer, cancel

    local pressInput     = nil
    local pressInputPos  = nil
    local dragUnlocked   = false  -- true once HOLD_TIME has elapsed
    local dragging       = false  -- true once drag has begun after unlock
    local cancelled      = false  -- true if finger drifted before timer ended
    local holdThread     = nil
    local currentDragPos = nil
    ...
    btn.InputBegan:Connect(function(inp)
        ...
        if pressInput then return end

        pressInput    = inp
        pressInputPos = Vector2.new(inp.Position.X, inp.Position.Y)
        dragUnlocked  = false
        dragging      = false
        cancelled     = false

        local ap = btn.AbsolutePosition
        currentDragPos = Vector2.new(ap.X, ap.Y)

        TweenService:Create(btn,     TI.fast, {BackgroundTransparency = 0.0}):Play()
        -- Outline thickness animates over HOLD_TIME as a visual progress indicator
        TweenService:Create(outline, TweenInfo.new(HOLD_TIME, Enum.EasingStyle.Linear), {Thickness = 5}):Play()

        -- After HOLD_TIME: unlock drag (unless launcher is locked)
        holdThread = task.delay(HOLD_TIME, function()
            holdThread = nil
            if savedLauncherLocked then return end
            dragUnlocked = true
        ...
        renderConn = RunService.RenderStepped:Connect(function()
            ...
            local raw = Vector2.new(pressInput.Position.X, pressInput.Position.Y)

            if not dragUnlocked then
                -- Timer still running: if finger drifts too far, cancel everything
                if not cancelled and (raw - pressInputPos).Magnitude > TAP_MAX_PX then
                    cancelled = true
            ...
            -- inp.Position is in safe-area coords; add inset to convert to
            -- full-screen GUI space (IgnoreGuiInset = true).
            local pos   = Vector2.new(raw.X + inset.X, raw.Y + inset.Y)
            local sz2   = btn.AbsoluteSize

            if not dragging then
                -- First frame after unlock: snap currentDragPos centered on finger
                -- so there is zero offset when drag starts.
                currentDragPos = Vector2.new(pos.X - sz2.X / 2, pos.Y - sz2.Y / 2)
                dragging = true
            end

            local vp = workspace.CurrentCamera.ViewportSize
            local tx = math.clamp(pos.X - sz2.X / 2, 0, vp.X - sz2.X)
            local ty = math.clamp(pos.Y - sz2.Y / 2, 0, vp.Y - sz2.Y)
            currentDragPos = currentDragPos:Lerp(Vector2.new(tx, ty), 0.72)
            btn.Position   = UDim2.new(0, currentDragPos.X, 0, currentDragPos.Y)
        end)
    end)
    ...
    UserInputService.InputEnded:Connect(function(inp)
        ...
        if not dragUnlocked then
            -- Released before HOLD_TIME: only open if finger didn't drift
            if not cancelled then
                doToggle()
            end
        else
            -- Released after HOLD_TIME → do NOT open, just save position
            dragging     = false
            dragUnlocked = false
            -- Inline save: saveLauncherPosition() is defined after buildLauncherCircle
            -- so it isn't a valid upvalue here; inline the logic instead.
...
local function recolorOutline(col)
    ...
    pcall(function()
        -- Always do a fresh lookup so the reference is never stale
        local sg  = LocalPlayer.PlayerGui:FindFirstChild("LuwaLauncher")
        local btn = sg and sg:FindFirstChildOfClass("TextButton")
        local stroke = btn and btn:FindFirstChildOfClass("UIStroke")
        if stroke then
            stroke.Color  = col
            launcherOutline = stroke
        elseif launcherOutline and launcherOutline.Parent then
            launcherOutline.Color = col
        end
    end)
end
...
local flingTargets       = {}
local _afTeardown        = nil
local flingPausedAntiFling = false

local AF_VEL_LIMIT       = 60
local AF_ANG_LIMIT       = 30
local AF_HRP_SIZE_MAX    = 5
...
local function stopAntiFling()
    ...
    if _afTeardown then pcall(_afTeardown); _afTeardown = nil end
...
local function startAntiFling()
    local gChar = LocalPlayer.Character
    local gHum  = gChar and gChar:FindFirstChildOfClass("Humanoid")
    if gHum then
        if gHum.WalkSpeed > AF_SPEED_THRESHOLD then
        ...
        if gHum.JumpPower > AF_JUMP_THRESHOLD then
    ...
    afHeartbeatConn = RunService.RenderStepped:Connect(function()
        if not antiFlingActive then return end
        pcall(function()
            local myChar = LocalPlayer.Character
            local myHum  = myChar and myChar:FindFirstChildOfClass("Humanoid")
            local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
            if not (myChar and myHum and myRoot and myHum.Health > 0) then return end
            ...
            for _, plr in ipairs(Players:GetPlayers()) do
                if plr ~= LocalPlayer and not flingTargets[plr.Name] then
                    ...
                    if hrp then
                        if hrp.AssemblyLinearVelocity.Magnitude > AF_VEL_LIMIT then
                            hrp.AssemblyLinearVelocity = Vector3.zero
                        end
                        if hrp.AssemblyAngularVelocity.Magnitude > AF_ANG_LIMIT then
                            hrp.AssemblyAngularVelocity = Vector3.zero
                        end
                        if hrp.Size.X > AF_HRP_SIZE_MAX
                        or hrp.Size.Y > AF_HRP_SIZE_MAX
                        or hrp.Size.Z > AF_HRP_SIZE_MAX then
...
local TABS = {
    {
        ...
        build = function(content)
            ...
            local function buildCommandsWindow()
                ...
                cmdWinSg.DisplayOrder = 1022
                cmdWinSg.Parent = LocalPlayer.PlayerGui

                local CW_W = savedCmdWinScale and savedCmdWinScale.w or 260
                local CW_H = savedCmdWinScale and savedCmdWinScale.h or 310
                local cwFrame = Instance.new("Frame")
                cwFrame.Size = UDim2.new(0, CW_W, 0, CW_H)
                if savedCmdWinPos then
                    cwFrame.Position = UDim2.new(savedCmdWinPos.xs, savedCmdWinPos.xo, savedCmdWinPos.ys, savedCmdWinPos.yo)
                else
                ...
                local CMD_LIST = {
                    ...
                    { cmd="serverhop",      desc="Hop to a random different server" },
                    ...
                    { cmd="esp",         desc="ESP all players, or esp [name] for one" },
                ...
                do
                    ...
                    UserInputService.InputEnded:Connect(function(inp)
                        if inp == _cwRDrag then
                            _cwRDrag = nil
                            local sz = cwFrame.Size
                            savedCmdWinScale = { w=sz.X.Offset, h=sz.Y.Offset }
                            patchInfo({ cmd_sw=sz.X.Offset, cmd_sh=sz.Y.Offset })
                        end
                    end)
                end
                ...
                do
                    ...
                    UserInputService.InputEnded:Connect(function(inp)
                        if inp == _cwDragInp then
                            _cwDragInp = nil
                            if saveWinPositions and cwFrame and cwFrame.Parent then
                                local p = cwFrame.Position
                                savedCmdWinPos = { xs=p.X.Scale, xo=p.X.Offset, ys=p.Y.Scale, yo=p.Y.Offset }
                                patchInfo({ cmd_xs=p.X.Scale, cmd_xo=p.X.Offset, cmd_ys=p.Y.Scale, cmd_yo=p.Y.Offset })
                            end
                        end
                    end)
                end
            ...
            local DESCRIPTIONS = {
                ...
                ["esp"]         = "esp — ESP all players | esp [name] — ESP a specific player by username or display name (partial ok)",
            ...
            local function doServerhop(mode)
                -- mode: nil = any, "high" = most players, "low" = fewest players
                logLine("> fetching servers...", rgb(160, 160, 175))
                task.spawn(function()
                    local ok, err = pcall(function()
                        ...
                        for _, s in ipairs(servers) do
                            if s.id ~= currentJob and s.id and s.id ~= "" then
                                candidates[#candidates+1] = s
                            end
                        end

                        if #candidates == 0 then
                            logLine("> no other servers found", rgb(255, 200, 60))
                            return
                        end
                        ...
                        logLine("> hopping to server (" .. (target.playing or "?") .. " players)...", rgb(50, 210, 100))
                        TP:TeleportToPlaceInstance(placeId, target.id, LocalPlayer)
                    end)
                    if not ok then
                        logLine("> serverhop failed: " .. tostring(err), rgb(255, 80, 80))
                    end
                end)
            end

            COMMANDS["serverhop"] = function(args)
                local sub = (args or ""):match("^%s*(.-)%s*$"):lower()
                if sub == "high" then
                    doServerhop("high")
                elseif sub == "low" then
                    doServerhop("low")
            ...
            local function addEspForPlayer(plr)
                ...
                local data = { conns = {}, char = char, hrp = hrp, hum = hum, plr = plr }
                espObjects[plr] = data

                -- 2D box frame (position/size driven by shared RenderStepped loop)
                ...
                -- ONE Highlight per character model instead of per-part.
                -- Same visual result, ~15x fewer instances.
                local hl = Instance.new("Highlight")
                hl.Adornee             = char
                hl.FillTransparency    = 1
                hl.OutlineTransparency = 0
                hl.OutlineColor        = Color3.fromRGB(255, 160, 60)
                hl.Enabled             = espSettings.showParts
                hl.Parent              = workspace
                data.highlight = hl

                -- BillboardGui for name/stats labels
                ...
                -- No per-player Heartbeat; labels are updated in the shared loop below.
            end
            ...
            local function enableEsp()
                ...
                espPlayerAddedConn = Players.PlayerAdded:Connect(function(plr)
                    -- CharacterAdded covers both first spawn and respawns
                    espCharAddedConns[plr] = plr.CharacterAdded:Connect(function()
                        task.defer(function() addEspForPlayer(plr) end)
                    end)
                    -- Also handle if character already exists by the time we connect
                ...
                -- Single shared loop: handles both 2D box (RenderStepped) and
                -- label text updates (throttled to every 4th frame = ~15 Hz).
                -- Box projection uses just 2 WorldToViewportPoint calls per player
                -- (HRP top + bottom) instead of 8 corners × all body parts.
                if not espBoxConn then
                    local espTick    = 0
                    local myHrpCache = nil
                    espBoxConn = RunService.RenderStepped:Connect(function()
                        espTick = espTick + 1
                        local doLabels = (espTick % 4 == 0)
                        local cam = workspace.CurrentCamera

                        if doLabels then
                            local myC = LocalPlayer.Character
                            myHrpCache = myC and myC:FindFirstChild("HumanoidRootPart")
                        end
                        ...
                        for _, data in pairs(espObjects) do
                            ...
                            local hum = data.hum
                            local plr = data.plr
                            -- no `continue` for executor compat: wrap in if
                            if hrp and hrp.Parent and hum and hum.Parent then

                                -- ── 2D box ────────────────────────────────────
                                if data.boxFrame then
                                    if not espSettings.showBox then
                                        data.boxFrame.Visible = false
                                    else
                                        local hrpPos       = hrp.Position
                                        local topSP, topIn = cam:WorldToViewportPoint(hrpPos + Vector3.new(0,  3.0, 0))
                                        local botSP, botIn = cam:WorldToViewportPoint(hrpPos + Vector3.new(0, -2.8, 0))
                                        local h = math.abs(botSP.Y - topSP.Y)
                                        -- topSP.Z > 0 = player is in front of camera (not behind).
                                        -- Either point in-viewport is enough; handles tall/close players
                                        -- where the top may be off-screen, and also far players.
                                        if topSP.Z > 0 and (topIn or botIn) and h > 2 then
                                            local w    = h * 0.42
                                            local cx   = (topSP.X + botSP.X) * 0.5
                                            local minY = math.min(topSP.Y, botSP.Y)
                                            data.boxFrame.Visible  = true
                                            data.boxFrame.Position = UDim2.new(0, cx - w - 2, 0, minY - 2)
                                            data.boxFrame.Size     = UDim2.new(0, w * 2 + 4,  0, h + 4)
                                        else
                                            data.boxFrame.Visible = false
                                        end
                                    end
                                ...
                                -- ── Labels: visibility mirrors settings toggles ─
                                -- Apply visibility each tick so toggling settings
                                -- in the ESP panel takes effect immediately.
                                if data.nameLbl  then data.nameLbl.Visible  = espSettings.showName     end
                                if data.hpLbl    then data.hpLbl.Visible    = espSettings.showHealth   end
                                if data.distLbl  then data.distLbl.Visible  = espSettings.showDistance end
                                if data.speedLbl then data.speedLbl.Visible = espSettings.showSpeed    end
                                if data.stateLbl then data.stateLbl.Visible = espSettings.showState    end
                                if data.highlight then data.highlight.Enabled = espSettings.showParts  end

                                -- ── Text updates: throttled to ~15 Hz ─────────
                                if doLabels then
                                    ...
                                    if espSettings.showHealth then
                                        ...
                                        local ratio = hum.Health / math.max(1, mhp)
                                        data.hpLbl.TextColor3 = Color3.fromRGB(
                                            math.floor(255*(1-ratio)), math.floor(220*ratio), 0)
                                        data.hpLbl.Text = hp .. " / " .. mhp .. " HP"
                                    end
                                    if espSettings.showDistance then
                                        data.distLbl.Text = (myHrpCache and
                                            math.floor((hrp.Position - myHrpCache.Position).Magnitude)
                                            or 0) .. " studs"
                                    end
                                    if espSettings.showSpeed then
                                        data.speedLbl.Text = math.floor(
                                            hrp.AssemblyLinearVelocity.Magnitude) .. " stud/s"
                                    end
                                    if espSettings.showState then
                                        data.stateLbl.Text = derivePlayerState(hum, hrp)
                                    end
                                end

                            end -- if hrp and hum valid
                        end
                    end)
                end
            end
            ...
            COMMANDS["esp"] = function(args)
                local query = args and args:match("^%s*(.-)%s*$") or ""

                -- ── "esp" with no argument: ESP all players ──────────────────
                if query == "" then
                    if espActive then
                ...
                -- ── "esp [name]": ESP a specific player ──────────────────────
                local q = query:lower()

                -- Collect every non-local player with their username + displayname
                local all = {}
                for _, plr in ipairs(Players:GetPlayers()) do
                    if plr ~= LocalPlayer then
                        all[#all+1] = {
                ...
                for _, e in ipairs(all) do
                    if e.user == q or e.display == q then
                        exactMatches[#exactMatches+1] = e.plr
                    end
                end

                -- helper: enable full ESP then strip everyone except the target
                local function espOnePlayer(target)
                    buildEspSettingsPanel()
                    if not espActive then
                        enableEsp()
                    end
                    -- remove everyone except our target so only they have ESP
                    for _, plr in ipairs(Players:GetPlayers()) do
                        if plr ~= target then
                            removeEspForPlayer(plr)
                        end
                    end
                    -- make sure the target has ESP (enableEsp may have added them already)
                    addEspForPlayer(target)
                    logLine("> ESP on: " .. target.Name, rgb(50, 210, 100))
                end

                if #exactMatches == 1 then
                    espOnePlayer(exactMatches[1])
                    return
                end

                -- 2. Multiple exact matches (two players share the same display name)
                if #exactMatches > 1 then
                    local names = {}
                    for _, p in ipairs(exactMatches) do names[#names+1] = p.Name end
                    logLine("> some player(s) also have this nickname — try typing their username ("
                        .. table.concat(names, ", ") .. ")", rgb(255, 200, 60))
                    return
                end

                -- 3. Prefix match on username or displayname
                local prefixMatches = {}
                for _, e in ipairs(all) do
                    if e.user:sub(1, #q) == q or e.display:sub(1, #q) == q then
                        prefixMatches[#prefixMatches+1] = e.plr
                    end
                end

                if #prefixMatches == 1 then
                    espOnePlayer(prefixMatches[1])
                    return
                end

                if #prefixMatches > 1 then
                    local names = {}
                    for _, p in ipairs(prefixMatches) do names[#names+1] = p.Name end
                    logLine("> some players have this word at the start of their nick/user — try typing their whole username ("
                        .. table.concat(names, ", ") .. ")", rgb(255, 200, 60))
                    return
                end

                -- 4. No match at all
                logLine("> no player found matching '" .. query .. "'"  , rgb(255, 80, 80))
            end
            ...
            local function buildServersWin()
                ...
                if savedSrvPos then
                    srvFrame.Position = UDim2.new(savedSrvPos.xs, savedSrvPos.xo, savedSrvPos.ys, savedSrvPos.yo)
                else
                ...
                do
                    ...
                    UserInputService.InputEnded:Connect(function(inp)
                        if inp == _dInp then
                            _dInp = nil
                            if saveWinPositions and srvFrame and srvFrame.Parent then
                                local p = srvFrame.Position
                                savedSrvPos = { xs=p.X.Scale, xo=p.X.Offset, ys=p.Y.Scale, yo=p.Y.Offset }
                                patchInfo({ srv_xs=p.X.Scale, srv_xo=p.X.Offset, srv_ys=p.Y.Scale, srv_yo=p.Y.Offset })
                            end
                        end
                    end)
                end
    ...
    },
    {
        ...
        build = function(content)
            ...
            rebuildWpBox = function()
                ...
                for idx, wp in ipairs(waypoints) do
                    ...
                    tapConnect(tpBtn, function()
                        ...
                        task.delay(0.3, function()
                            if wasActive and antiFlingOpt and antiFlingOpt.state then
                                startAntiFling()
                            end
                        end)
            ...
            local function showPicker()
                ...
                buildPickerCards()
                -- Restore saved size, otherwise default
                if savedPickerScale then
    ...
    },
    {
        ...
        build = function(content)
            ...
            makeButton(sf, {
                name="Reset Position", hint="Moves the icon back to its default spot",
                action=function()
                    local sz2 = savedLauncherSize or 44
                    savedLauncherPos = nil
                    patchInfo({ launcher_xs="", launcher_xo="", launcher_ys="", launcher_yo="" })
                    if launcherBtn and launcherBtn.Parent then
                        TweenService:Create(launcherBtn, TI.med,
                            {Position = UDim2.new(1, -sz2 - 14, 0.5, -sz2/2)}):Play()
                    end
                end,
            })
            ...
            local function enableRescale()
                ...
                activeSaveGui.DisplayOrder = 1040
            ...
            makeToggle(sf, {
                ...
                hint  = "Remembers positions of all windows",
                default = saveWinPositions,
                state   = saveWinPositions,
                enable  = function()
                    saveWinPositions = true
                    patchInfo({ save_win_pos = "true" })
                end,
                disable = function()
                    saveWinPositions = false
                    savedPosition   = nil
                    ...
                    patchInfo({
                        save_win_pos = "false",
                        pos_xs=DELETE,     pos_xo=DELETE,     pos_ys=DELETE,     pos_yo=DELETE,
                        picker_xs=DELETE,  picker_xo=DELETE,  picker_ys=DELETE,  picker_yo=DELETE,
                        profile_xs=DELETE, profile_xo=DELETE, profile_ys=DELETE, profile_yo=DELETE,
                        cmd_xs=DELETE,     cmd_xo=DELETE,     cmd_ys=DELETE,     cmd_yo=DELETE,
                        srv_xs=DELETE,     srv_xo=DELETE,     srv_ys=DELETE,     srv_yo=DELETE,
                    })
                end,
            })
            ...
            makeToggle(sf, {
                ...
                disable = function()
                    ...
                    savedProfileScale = nil
                    savedCmdWinScale  = nil
                    patchInfo({ save_win_sizes = "false", picker_sw=DELETE, picker_sh=DELETE, profile_sw=DELETE, profile_sh=DELETE, cmd_sw=DELETE, cmd_sh=DELETE })
                end,
            })
...
function createUI()
    ...
    do
        ...
        UserInputService.InputEnded:Connect(function(inp)
            ...
            if saveWinPositions and Frame and Frame.Parent then
